...
extract( $atts );
$addresses = (array) vc_param_group_parse_atts( $addresses );
...
foreach ( $addresses as $address ) {
	$adds[] = trim( esc_html( $address['address'] ) );
}
$addstr = implode( '|', $adds );
?>
<div <?php
if ( $el_class ) {
	echo 'class="thememove-gmaps-multi ' . $el_class . '"';
} else {
	echo 'id="map-canvas-multi" class="thememove-gmaps-multi"';
}
?>
	data-address="<?php echo esc_html( $addstr ); ?>"
	...
	jQuery(document).ready(function ($) {
		...
		(function ($) {
			...
			if (gmMapDiv.length) {
				...
				var address_arr = [];
				var adds = gmMarkerAddress.split('|');
